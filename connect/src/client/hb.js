...
function toSigBaseArgs({ url, method, headers, includePath = false }) {
  ...
  return {
     ...
     */
    fields: [...headers.keys(), ...(includePath ? ["@path"] : [])].sort(),
  ...
  return args => {
    return (
      of(args)
         ...
         */
        .chain(
          fromPromise(({ processId, data, tags }) =>
            encodeDataItem({ processId, data, tags }),
          ),
        )
        .chain(
          fromPromise(({ headers, body }) => {
        ...
        .chain(request =>
          of(request)
            .chain(
              fromPromise(({ url, method, headers, body }) =>
  ...
  return args => {
    return (
      of(args)
         ...
         */
        .chain(
          fromPromise(({ processId, data, tags, anchor }) =>
...
export function loadResultWith({ fetch, logger: _logger, HB_URL, signer }) {
  ...
  return args => {
    return of(args)
      .chain(
        fromPromise(async ({ id, processId }) => {
          const { headers, body } = await encodeDataItem({ processId })
...
export function relayerWith({ fetch, logger, HB_URL, signer }) {
  ...
  return async (url, options) => {
    ...
    const { headers: signedHeaders } = await toHttpSigner(signer)({
      fields: [...options.headers.keys(), "relay-path"].sort(),
      request: {
        url,
        ...options,
        headers: {
